...
module.exports = function () {
    ...
    module.execute = function (arrParams) {
        return new Promise(function (resolve, reject) {
            ...
            var argsFile = path.join(__dirname, '/jar/args.txt')

            // Dump arguments into the file
            console.log("Writing arguments...")
            fs.writeFile(argsFile, arrParams.join(' '), (err) => {
                if (!err)
                    console.log('Done')
                else {
                    console.log(`Error saving args to file: ${err.message}`)
                }
            })
